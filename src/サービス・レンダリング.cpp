...
namespace エンジン
{
	...
	void レンダリングサービス::描画_リサイズ(unsigned int リソースID, int x, int y, float サイズ) {
		...
		GetGraphSize(ローダー->ハンドル所得(uv.画像), &画像サイズx, &画像サイズy);

		//画像サイズ調整
		if (サイズ != 0) {
			if (サイズ <= 0)画像サイズx /= -サイズ, 画像サイズy /= -サイズ;
			if (サイズ > 0)画像サイズx *= サイズ, 画像サイズy *= サイズ;
		}

		//画像配置
		画像_1x = (画像サイズx / 2) - 画像サイズx;
		画像_2x = (画像サイズx / 2) + 画像サイズx;
		画像_1y = (画像サイズy / 2) - 画像サイズy;
		画像_2y = (画像サイズy / 2) + 画像サイズy;

		DrawExtendGraph(画像_1x + x, 画像_1y + y, 画像_2x + x, 画像_2y + y, ローダー->ハンドル所得(uv.画像), TRUE);
	}
}
